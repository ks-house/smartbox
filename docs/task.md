...
- [x] **1단계: 환경 설정 및 HAL 구조 수립**
  - [x] [platformio.ini](../platformio.ini)에 `[env:native]` 테스트 실행 환경 설정 추가
  - [x] [HardwareInterface.h](../src/HardwareInterface.h) [NEW]: 추상 하드웨어 API 정의 (GPIO 제어, 시간 측정, 센서 인터페이스)
  - [x] [MockHardware.h](../src/MockHardware.h) [NEW]: 유닛 테스트 검증용 모의 객체(Mock) 구현 (가상의 시간 흐름, 핀 상태 기록, 센서 피드백 조작 기능)
  - [x] [Esp32Hardware.h](../src/Esp32Hardware.h) [NEW], [Esp32Hardware.cpp](../src/Esp32Hardware.cpp) [NEW]: 아두이노 프레임워크 및 실제 하드웨어 바인딩 구현

- [x] **2단계: TDD 1 - 부팅 시 릴레이 글리치 차단 및 초기 안전 상태 검증**
  - [x] `test/test_native/test_main.cpp`에 [RED] 유닛 테스트 작성: `SmartBoxController::init()` 시점에 릴레이 세 핀(`GPIO 6, 7, 8`)이 즉시 `INPUT`(고임피던스) 상태로 설정되는지 검증
  - [x] [SmartBoxController.cpp](../src/SmartBoxController.cpp) [NEW] 구현을 통해 [GREEN] 테스트 통과: 아키텍처에 정의된 글리치 방지 모드로 핀 초기 설정
  - [x] [REFACTOR] 핀 정의 및 상수 매핑 데이터 구조 정리

- [x] **3단계: TDD 2 - 릴레이 인터락 및 100ms 가드 타임 검증**
  - [x] `test/test_native/test_main.cpp`에 [RED] 유닛 테스트 작성:
  ...
  - [x] [SmartBoxController.cpp](../src/SmartBoxController.cpp) 구현을 통해 [GREEN] 테스트 통과: `millis()` 기반 비동기 대기 상태 전이 구현
  - [x] [REFACTOR] 지연 및 상태 제어 함수 모듈화로 가독성 향상

- [x] **4단계: TDD 3 - 과전류 감지 시 비상 정지(EMERGENCY_STOP) 검증**
  - [x] `test/test_native/test_main.cpp`에 [RED] 유닛 테스트 작성:
  ...
  - [x] [SmartBoxController.cpp](../src/SmartBoxController.cpp) 구현을 통해 [GREEN] 테스트 통과: 과전류 모니터링 스레드 시뮬레이션 및 상태 전이 구현
  - [x] [REFACTOR] 전류 임계치 및 기동 시간 상수를 제어기 내 설정 변수(`BoxConfig`)로 추상화

- [x] **5단계: TDD 4 - 저전압 셧다운(BATTERY_LOW_SHUTDOWN) 검증**
  - [x] `test/test_native/test_main.cpp`에 [RED] 유닛 테스트 작성:
  ...
  - [x] [SmartBoxController.cpp](../src/SmartBoxController.cpp) 구현을 통해 [GREEN] 테스트 통과: 저전압 축적 타이머 및 동결 시퀀스 구현
  - [x] [REFACTOR] 누적 시간 카운팅 로직의 모듈성 확보

- [x] **6단계: TDD 5 - 초음파 거리 센서 중간값 필터 검증**
  - [x] `test/test_native/test_main.cpp`에 [RED] 유닛 테스트 작성:
    - 50ms 마다 수집되는 최근 5개의 초음파 거리 원시 데이터 중 일시적인 오인식 노이즈(튀는 값)가 들어와도 필터를 거친 중간값에 의해 정확히 상태 전이가 결정되는지 검증
  - [x] [SmartBoxController.cpp](../src/SmartBoxController.cpp) 구현을 통해 [GREEN] 테스트 통과: 최근 5개 샘플에 대한 중간값(Median) 정렬 알고리즘 적용
  - [x] [REFACTOR] 삽입 정렬 등 효율적인 버퍼 핸들링 리팩토링

---

## Phase 2: 설정 저장 및 이벤트 아키텍처 (TDD 확장)

- [x] **7단계: TDD 6 - 감도 조절 동적 변경 및 이벤트 콜백 리포트 검증**
  - [x] `test/test_native/test_main.cpp`에 [RED] 유닛 테스트 작성:
  ...
  - [x] [SmartBoxController.cpp](../src/SmartBoxController.cpp) 구현을 통해 [GREEN] 테스트 통과: `BoxConfig` 변경 바인딩 및 상태 천이 함수 내 콜백 트리거 코드 추가
  - [x] [REFACTOR] 제어기 내에서 설정 및 콜백 처리 구조 최적화
...
- [x] **11단계: 메인 통합**
  - [x] [main.cpp](../src/main.cpp) [MODIFY]:
    - [x] `Esp32Hardware` 인스턴스화 및 `SmartBoxController` 생성
    - [x] `ConfigManager`를 통한 초기 세팅 인입
    - [x] `WifiManager` 및 `SmartThingsReporter` 구동 및 FSM 상태 콜백 주입
    - [x] 비동기 웹서버 시작
  - [x] 전체 빌드 및 Native 테스트 재수행 검증
- [x] **12단계: 타겟 보드 업로드 및 실물 검증**
